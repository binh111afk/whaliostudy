...
const DOM = {
    ...
    // --- SỬA ĐOẠN NÀY ---
    // 1. TẠO MỘT NHÓM MỚI CHO HEADER
    actions: {
        guestActions: document.getElementById('guest-actions'),
        userActions: document.getElementById('user-actions'),
    },

    // 2. XÓA 2 DÒNG guestActions VÀ userActions KHỎI SECTIONS
    sections: {
        // guestActions: ... (XÓA DÒNG NÀY)
        // userActions: ... (XÓA DÒNG NÀY)
...
const Auth = {
    ...
    // Handle logout
    logout() {
        StatsManager.stopStudyTimer(); // <--- THÊM DÒNG NÀY (Dừng đếm giờ của user cũ)
        AppState.clearUser();
        location.reload();
    }
};
...
const UI = {
    ...
    showUserInterface(user) {
        // SỬA DOM.sections THÀNH DOM.actions
...
// ==================== DOCUMENT MANAGER (FIX LỖI POPUP XÓA) ====================
const DocumentManager = {
    ...
    async loadCourses() {
        try {
            // Danh sách môn học (không có icons)
            AppState.allCourses = [
    ...
    // 2. Render Giao diện (Tự động chọn nơi vẽ)
    renderPagedDocuments() {
        ...
        if (docsToShow.length === 0) {
            this.renderEmptyState(
                AppState.isViewingSaved ? "Bạn chưa lưu tài liệu nào" : "Không tìm thấy tài liệu",
    ...
    // 3. Tạo HTML Thẻ Card (Chuẩn Style cũ)
    generateCardsHTML(docs) {
        ...
        return docs.map(doc => {
            ...
            // Nút xóa Admin (Dùng class .admin-delete-btn)
                ...
                <div class="doc-card" data-id="${doc.id}" onclick="window.open('${doc.path}', '_blank')">
                    ...
                    <div class="doc-card-header">
                        ...
                        <div class="doc-card-info">
                            ...
                            <div class="doc-uploader">👤 ${Utils.escapeHtml(doc.uploader)}</div>
                    ...
                    <div class="doc-card-body">
                        <div class="doc-meta-info">
                            <div class="doc-meta-item">📅 ${uploadDate}</div>
                            ${doc.size ? `<div class="doc-meta-item">💾 ${Utils.formatFileSize(doc.size)}</div>` : ''}
                        </div>
    ...
    // --- 4. HỆ THỐNG XỬ LÝ MODAL XÓA (ĐÃ FIX) ---
    ...
    openDeleteModal(docId, username) {
        ...
        // Tìm trực tiếp bằng ID (Không dùng biến DOM cũ)
        const modal = document.getElementById('deleteDocConfirmModal');
        if (modal) modal.classList.add('active');
        else console.error("Lỗi: Không tìm thấy Modal deleteDocConfirmModal");
    },
    ...
    closeDeleteModal() {
        ...
        // Tìm trực tiếp để đóng
        const modal = document.getElementById('deleteDocConfirmModal');
        if (modal) modal.classList.remove('active');
    },

    // Xác nhận Xóa (Nút Xóa ngay)
    async confirmDeleteAction() {
        if (!this.pendingDocId) {
            // Trường hợp lỗi logic, đóng modal luôn
            this.closeDeleteModal();
            return;
        }

        // Tìm Modal và Nút bấm để hiển thị loading
        const modal = document.getElementById('deleteDocConfirmModal');
        // Tìm nút "Xóa ngay" (thường là nút thứ 2 trong modal)
        const buttons = modal ? modal.querySelectorAll('button') : [];
        const btnDelete = buttons.length > 0 ? buttons[buttons.length - 1] : null; // Lấy nút cuối cùng
        ...
        try {
            ...
            // Trả lại trạng thái nút (đề phòng mở lại lần sau)
            if(btnDelete) {
    ...
    toggleSave: async function(docId) { /* Logic cũ */ 
        ...
        } catch(e) { Utils.showAlert("Lỗi", "Không thể lưu", false); }
    },
    trackDownload: function(docId) { console.log('DL', docId); },
    updateStats: function() { 
        // Tính dung lượng lưu trữ từ danh sách tài liệu
        let totalSize = 0;
        let fileSize = 0;
        let imageSize = 0;
        ...
        if (fileSizeEl) fileSizeEl.textContent = `📁 Files: ${fileSizeMB.toFixed(1)} MB`;
        if (imageSizeEl) imageSizeEl.textContent = `🖼️ Ảnh: ${imageSizeMB.toFixed(1)} MB`;
    },
...
// ==================== PAGE MANAGEMENT ====================
const PageManager = {
    // Hàm ẩn tất cả (Cập nhật để tắt chế độ full-mode)
    hideAllSections() {
        Object.values(DOM.sections).forEach(section => {
            if (section) section.style.display = 'none';
        });

        // Ẩn section thư viện riêng
        const libSection = document.getElementById('library-section');
        if (libSection) libSection.style.display = 'none';

        // QUAN TRỌNG: Tắt chế độ full màn hình (để hiện lại menu/sidebar khi về trang chủ)
    ...
    showDashboard() {
        ...
        // Lấy link Trang chủ từ sidebar-left
    ...
    // --- HÀM MỚI: Dành riêng cho nút Tài liệu trên Thanh điều hướng ---
    // Trong PageManager
    ...
    // Hàm cũ cho Widget bên phải (Giữ nguyên logic cũ)
    showDocumentsPage(menuItem) {
        ...
        // Thiết lập chế độ render cũ
        DocumentManager.currentMode = 'normal'; // Chế độ Widget (Lưới nhỏ)
        DocumentManager.loadAllDocuments();
    },
    ...
    showSavedDocumentsPage(menuItem = null) {
        ...
        // ✅ ĐẶT TITLE CHO TRANG TÀI LIỆU ĐÃ LƯU
...
// ==================== 10. STATS MANAGER (FIX LỖI TRÙNG USER) ====================
const StatsManager = {
    timerInterval: null, // Lưu biến interval để có thể dừng lại khi logout

    // Hàm tạo key riêng cho từng user
    getUserKey(baseKey) {
        const username = AppState.currentUser ? AppState.currentUser.username : 'guest';
        return `${baseKey}_${username}`; // Ví dụ: whalio_streak_admin
    },

    init() {
        this.stopStudyTimer(); // Dừng timer cũ nếu có
        this.checkStreak();
        this.startStudyTimer();
        this.updateUI();
    },

    // 1. Logic tính Streak (Chuỗi ngày)
    checkStreak() {
    ...
    // 2. Logic tính Giờ học
    startStudyTimer() {
        const keyTotalTime = this.getUserKey('whalio_total_minutes');

        // Cứ mỗi 1 phút thì tăng biến đếm lên 1
        this.timerInterval = setInterval(() => {
            // Luôn lấy key mới nhất trong interval phòng trường hợp user đổi mà không reload
    ...
    updateUI() {
        ...
        const streak = localStorage.getItem(keyStreak) || 1; // Mặc định là 1 nếu chưa có
...
// ==================== EXAM MANAGER (QUẢN LÝ THI ONLINE) ====================
const ExamManager = {
    // Dữ liệu giả lập (Mock Data)
    examsData: [
        {
            id: 1,
            title: "Đề ôn tập Pháp Luật Đại Cương - Chương 1",
            desc: "Tổng hợp các câu hỏi trắc nghiệm về Nhà nước và Pháp luật, phù hợp ôn thi giữa kỳ.",
            image: "./img/pldc-pic.png", // Dùng lại ảnh bạn đã có
            subject: "Pháp luật",
            questions: 40,
            time: "60 phút"
        },
        {
            id: 2,
            title: "Trắc nghiệm Tâm Lý Học - Tính cách & Khí chất",
            desc: "Bài kiểm tra kiến thức về các loại khí chất, tính cách con người và hành vi ứng xử.",
...
const EventHandlers = {
    ...
    async handleUploadDocument(event) {
        ...
        let finalType = 'other'; // Mặc định là other

        // --- LOGIC: TỰ ĐỘNG ÉP KIỂU DỰA TRÊN ĐUÔI FILE (IGNORE USER SELECTION) ---
        ...
        // QUAN TRỌNG: Thay vì lấy từ dropdown, ta lấy từ biến finalType đã tính toán ở trên
        ...
        if (result.success) {
            ...
            ModalManager.close('uploadDoc');

            // Add new document to the list
            AppState.allDocuments.unshift(result.document);
            DocumentManager.renderAllDocuments();
            DocumentManager.updateStats();
    ...
    setupMenuEvents() {
        ...
        // Profile dropdown toggle
        {
            document.addEventListener('click', (e) => {
                // Nếu click vào Avatar (nút kích hoạt) thì DỪNG LẠI, đừng đóng menu
                // (Vì hàm toggleUserMenu đã lo việc mở/đóng rồi)
                if (e.target.closest('.nav-avatar') || e.target.closest('.avatar-wrapper')) {
                    return;
                }

                // Chỉ đóng khi click ra ngoài menu
        ...
        if (docFileInput) {
            docFileInput.addEventListener('change', function () {
                if (this.files && this.files[0]) {
                    ...
                    // 1. Tiện tay điền luôn tên file vào ô nhập tên (nếu ô tên đang trống)
                    // Giúp người dùng đỡ phải gõ lại tên
                    if (!nameInput.value) {
                        // Lấy tên file bỏ đi phần đuôi (ví dụ: Bai-tap.docx -> Bai-tap)
                        nameInput.value = this.files[0].name.replace(/\.[^/.]+$/, "");
                    }

                    // 2. Logic tự động chọn loại file dựa trên đuôi
                    if (fileName.endsWith('.pdf')) {
                        ...
                        // Lưu ý: Cần thêm option Excel vào HTML nếu chưa có
                        // Nếu HTML chưa có value='excel' thì nó sẽ nhảy về mặc định
                        // Kiểm tra lại file index.html dòng 750 xem có option excel chưa nhé
...
function initializeApp() {
    ...
    console.log('✅ study.js đã được tải thành công!');
}
...
// Tạo alias cho các hàm cũ để tương thích với HTML
...
// ==================== BỔ SUNG TÍNH NĂNG SỬA HỒ SƠ (Dán vào cuối file) ====================

// 1. Hàm tự động điền dữ liệu cũ vào form khi mở Modal
function fillEditProfileForm() {
    const user = AppState.currentUser;
    if (!user) return;

    // Helper: Tìm và gán giá trị nếu element tồn tại
    const setVal = (id, val) => {
        const el = document.getElementById(id);
        if (el) el.value = val || '';
    };

    // Điền dữ liệu vào 10 trường
    ...
    setVal('edit-gender', user.gender || 'Nam'); // Mặc định là Nam nếu chưa có
...
// ==================== CẬP NHẬT HỒ SƠ (PHIÊN BẢN GỌI API SERVER) ====================
...
window.handleUpdateProfile = async function (event) {
    ...
    // QUAN TRỌNG: Server cần "username" để biết đang sửa ai
    const profileData = {
        username: AppState.currentUser.username,
    ...
    try {
        // 2. Gửi dữ liệu lên Server Node.js
        // API này đã được viết sẵn trong file server.js của bạn
        ...
        if (result.success) {
            // 3. Nếu Server báo thành công (đã ghi vào json)

            // Cập nhật lại AppState (Bộ nhớ trình duyệt) để đồng bộ với Server
            // Giữ lại các thông tin cũ (như id, savedDocs, avatar) và ghi đè thông tin mới
            ...
            Utils.showAlert("Thành công", "Hồ sơ đã được lưu vào hệ thống!", true);
        ...
        Utils.showAlert("Lỗi", error.message || "Không thể kết nối tới server", false);
    } finally {
        Utils.setButtonLoading(btnSubmit, false);
    }
};

// Đảm bảo Window nhận diện hàm
if (window.App) {
    window.App.handleUpdateProfile = window.handleUpdateProfile;
}
// 3. GHI ĐÈ lệnh mở Modal cũ để nó gọi hàm fill dữ liệu trước khi mở
window.openEditProfileModal = () => {
    fillEditProfileForm(); // <--- Điền dữ liệu cũ vào trước
    ModalManager.open('editProfile'); // <--- Sau đó mới mở modal lên
};

// Cập nhật cả trong window.App để đồng bộ
if (window.App) {
    window.App.openEditProfileModal = window.openEditProfileModal;
}

// 4. Hàm xử lý xem trước ảnh khi chọn file (Avatar)
...
// ==================== TÍNH NĂNG ĐỔI MẬT KHẨU (PHIÊN BẢN GỌI API NODE.JS) ====================
...
window.handleChangePassword = async function (event) {
    ...
    if (!oldPass || !newPass || !confirmPass) {
        Utils.showAlert("Lỗi", "Vui lòng nhập đầy đủ các trường!", false);
        return;
    }

    if (newPass !== confirmPass) {
        Utils.showAlert("Lỗi", "Mật khẩu mới xác nhận không khớp!", false);
        return;
    }

    if (newPass.length < 6) {
        Utils.showAlert("Lỗi", "Mật khẩu mới phải có ít nhất 6 ký tự!", false);
        return;
    }

    // 3. Gửi yêu cầu lên Server Node.js
    Utils.setButtonLoading(btnSubmit, true);

    try {
        // Gọi API /api/change-password (đã có trong server.js của bạn)
        const response = await fetch('/api/change-password', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                username: AppState.currentUser.username, // Lấy username từ user đang đăng nhập
                oldPass: oldPass,
                newPass: newPass
            })
        });
        ...
        if (result.success) {
            // A. Đổi thành công
            Utils.showAlert("Thành công", "Đổi mật khẩu thành công! Vui lòng đăng nhập lại.", true);
            ModalManager.close('changePass');
            event.target.reset(); // Xóa trắng form

            // Cập nhật lại mật khẩu trong LocalStorage client để tránh bị lỗi nếu chưa logout ngay
            if (AppState.currentUser) {
                AppState.currentUser.password = newPass;
                AppState.saveUser(AppState.currentUser);
            }

            // Tự động đăng xuất sau 1.5 giây để bảo mật
            setTimeout(() => {
                Auth.logout();
            }, 1500);

        } else {
            // B. Đổi thất bại (VD: Sai mật khẩu cũ)
            Utils.showAlert("Thất bại", result.message || "Có lỗi xảy ra", false);
        }

    } catch (error) {
        console.error("Lỗi kết nối:", error);
        Utils.showAlert("Lỗi mạng", "Không thể kết nối tới server!", false);
    } finally {
        Utils.setButtonLoading(btnSubmit, false);
    }
};

// Đảm bảo window nhận diện hàm này
if (window.App) {
...
window.handleRegister = async function (event) {
    ...
    if (!fullName || !email || !username || !password || !confirmPassword) {
        Utils.showAlert("Lỗi", "Vui lòng điền đầy đủ thông tin!", false);
        Utils.setButtonLoading(btnSubmit, false);
        return;
    }

    if (password !== confirmPassword) {
        Utils.showAlert("Lỗi", "Mật khẩu nhập lại không khớp!", false);
        Utils.setButtonLoading(btnSubmit, false);
        return;
    }

    if (password.length < 6) {
        Utils.showAlert("Lỗi", "Mật khẩu phải có ít nhất 6 ký tự!", false);
        Utils.setButtonLoading(btnSubmit, false);
        return;
    }

    // 3. Gửi dữ liệu lên Server Node.js
    try {
        ...
        if (result.success) {
            ...
            Utils.showAlert("Thành công! 🎉", "Chào mừng bạn gia nhập Whalio Group!", true);

            // Xóa trắng form
            event.target.reset();

            // Tải danh sách tài liệu (để cập nhật trạng thái đã lưu nếu có)
            DocumentManager.loadAllDocuments();
        } else {
            // Server báo lỗi (VD: Tên đăng nhập đã tồn tại)
...
// Đảm bảo window nhận diện được hàm này
if (window.App) {
...
DocumentManager.deleteDocument = async function (docId) {
    ...
    try {
        const response = await fetch('/api/delete-document', {
            ...
            body: JSON.stringify({
                docId: docId,
                username: AppState.currentUser.username // Gửi kèm username để server check quyền
            })
        });
        ...
        if (result.success) {
            ...
            Utils.showAlert("Lỗi", result.message, false);
        }
    } catch (error) {
        console.error(error);
        Utils.showAlert("Lỗi", "Không kết nối được server", false);
    }
}

// ==================== FOCUS MODE LOGIC (WOW EDITION) ====================
...
const StudyTimer = {
    ...
    init() {
        ...
        // Chỉ update display nếu overlay tồn tại
    ...
    open() {
        ...
        // ✅ CẬP NHẬT TITLE NGAY KHI VÀO STUDYTIME
    ...
    exit() {
        ...
        // ✅ QUAN TRỌNG: ĐẶT LẠI TITLE VỀ DASHBOARD
        document.title = 'Whalio - Dashboard';

        // Quay lại Dashboard
        if (PageManager && PageManager.showDashboard) {
            PageManager.showDashboard();
        }
    },
    ...
    updateDisplay() {
        ...
        // ✅ CHỈ CẬP NHẬT TITLE KHI ĐANG Ở CHỂ ĐỘ FOCUS
...
// Đảm bảo hàm này nằm ở cấp cao nhất của file study.js, không nằm trong hàm khác
window.showStudyTimePage = function (el) {
    ...
    // Active menu sidebar - el là element <a> từ sidebar
    if (UI && UI.setActiveMenu) {
        UI.setActiveMenu(el || null);
    }

    // Khởi tạo lại StudyTimer
...
let selectedMinutes = 25; // Biến lưu thời gian được chọn
...
// Cập nhật hiển thị số phút khi kéo slider
...
// Cập nhật màu slider (gradient progress)
...
// Highlight preset button đang active
function highlightActivePreset(minutes) {
    ...
    // Tìm và highlight nút tương ứng
    presetButtons.forEach(btn => {
...
// Đặt thời gian từ preset button
...
// Xác nhận và áp dụng thời gian
function applyCustomTime() {
    if (selectedMinutes && selectedMinutes > 0) {
        // Gọi hàm setTime của StudyTimer
...
function scrollDocs(direction) {
    ...
    const scrollAmount = 300; // Độ dài mỗi lần bấm cuộn
    if (direction === 'left') {
...
// ==================== PROFILE MANAGER (XỬ LÝ TAB HỒ SƠ) ====================
const ProfileManager = {
    ...
    // Render Tài liệu của tôi (Do user upload)
    renderMyDocs() {
        const container = document.getElementById('profile-my-docs-list');
        if (!container || !AppState.currentUser) return;

        // Lọc tài liệu: Người upload trùng tên User hiện tại
        // Lưu ý: Lúc upload bạn lưu là fullName, nên so sánh theo fullName
        ...
        if (myDocs.length === 0) {
            ...
            // Tái sử dụng hàm render có sẵn của DocumentManager
            DocumentManager.renderDocuments(myDocs, container);
        }
    },

    // Render Tài liệu đã lưu
    renderSavedDocs() {
        ...
        // Lọc tài liệu: ID nằm trong danh sách đã lưu
