...
export const createBaseNode = ({
  ...
  data: {
    ...
    color: branchColor || getBranchColor(branchIndex),
    theme,
    direction,
    parentId,
    layoutDirection,
...
export const serializeNode = (node) => ({
  ...
  data: {
    ...
    color: String(node.data?.color || node.data?.branchColor || getBranchColor(node.data?.branchIndex ?? node.data?.colorIndex ?? 0)),
