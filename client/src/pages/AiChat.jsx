...
import { 
  Send, Paperclip, User, Plus, MessageSquare, 
  MoreVertical, Trash2, X, Image as ImageIcon, Copy, Check, Sparkles,
  ChevronDown, Menu
} from 'lucide-react';
...
const AiChat = () => {
  ...
  const handleSelectSession = async (sessionId) => {
    ...
    try {
      ...
      const res = await fetch(`/api/session/${sessionId}?username=${currentUsername}`);
      const data = await res.json();
      ...
      if (data.success && data.session) {
        ...
        const formattedMessages = data.session.messages.map(msg => ({
          ...
          image: msg.hasAttachment && msg.attachmentType === 'image' ? msg.attachmentUrl : null
        }));
  ...
  const handleDeleteSession = async (sessionId, e) => {
    ...
    try {
      const res = await fetch(`/api/session/${sessionId}?username=${user.username}`, { method: 'DELETE' });
      const data = await res.json();
      if (data.success) {
  ...
  const CodeBlock = ({ inline, className, children, ...props }) => {
    ...
    return !inline && match ? (
      ...
      <code className={`${className} bg-gray-100 text-red-600 px-1 py-0.5 rounded text-sm font-mono border border-gray-200`} {...props}>
        {children}
      </code>
    );
  };
  ...
  return (
    <div className="absolute inset-0 flex bg-white text-gray-800 overflow-hidden">
      ...
      <div className={`${sidebarOpen ? 'w-64' : 'w-0'} bg-white border-r border-gray-200 transition-all duration-300 flex flex-col h-full shrink-0 overflow-hidden`}>
        <div className="p-4 shrink-0 border-b border-gray-100">
          ...
          <button 
            ...
            className="w-full flex items-center justify-center gap-2 bg-white hover:bg-gray-50 text-blue-600 px-4 py-3 rounded-xl border border-dashed border-gray-300 hover:border-blue-500 transition-all font-bold shadow-sm group"
          >
            <Plus size={18} className="group-hover:rotate-90 transition-transform" /> Đoạn chat mới
          </button>
        </div>

        <div className="flex-1 min-h-0 overflow-y-auto px-3 space-y-1 py-4">
          {sessions.length === 0 ? (
            <div className="text-center text-gray-400 mt-10 text-sm px-3">Chưa có lịch sử</div>
          ) : (
            sessions.map(session => (
              ...
              >
                ...
                <span className="truncate flex-1 min-w-0">{session.title || "Cuộc trò chuyện mới"}</span>
                <button 
      ...
      <div className="flex-1 flex flex-col min-h-0 h-full">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <span className="font-bold text-gray-800 text-lg flex items-center gap-2">
                Whalio AI <Sparkles size={16} className="text-yellow-500 fill-yellow-500" />
              </span>
        ...
        {showScrollButton && (
          <button
            ...
            className="absolute bottom-24 left-1/2 transform -translate-x-1/2 bg-blue-600 text-white px-4 py-2 rounded-full shadow-lg hover:bg-blue-700 transition-all flex items-center gap-2 animate-bounce z-50"
          >
            <ChevronDown size={16} />
            Tin nhắn mới
          </button>
          ...
          {messages.map((msg) => (
            <div key={msg.id} className={`flex gap-4 max-w-4xl mx-auto ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
              ...
              <div className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 shadow-sm border ${
                ...
                {msg.role === 'model' ? <Sparkles size={18} className="fill-blue-600"/> : <User size={18} />}
              </div>

              {/* Message Content */}
              <div className="group relative max-w-[85%] min-w-0">
                <div className={`text-xs font-bold mb-1 ${msg.role === 'user' ? 'text-right text-gray-400' : 'text-left text-blue-600'}`}>
                  {msg.role === 'model' ? 'Whalio AI' : 'Bạn'}
                </div>

                <div className={`px-6 py-4 rounded-2xl shadow-sm leading-relaxed text-[15px] break-words max-w-full ${
                  msg.role === 'user' 
          ...
          <div className="max-w-4xl mx-auto p-4 relative">
            ...
            <div className="flex items-end gap-2 bg-white p-2 rounded-2xl border border-gray-200 focus-within:border-blue-500/50 focus-within:ring-4 focus-within:ring-blue-500/10 transition-all shadow-sm hover:shadow-md">
              ...
              <button 
                ...
                className={`p-3 rounded-xl transition-colors shrink-0 ${selectedFile ? 'text-blue-600 bg-blue-50' : 'text-gray-400 hover:text-blue-600 hover:bg-gray-50'}`}
                title="Gửi ảnh"
              >
                {selectedFile ? <ImageIcon size={22}/> : <Paperclip size={22}/>}
              </button>
              ...
              <textarea 
                ...
                placeholder="Nhập tin nhắn cho Whalio..."
